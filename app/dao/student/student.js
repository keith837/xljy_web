...
Student.queryPageByStudentId = function(studentId, startDate, start, pageSize, callback){
    ...
    sql += "UNION SELECT B.objId, 2 as dataType, B.attendanceId, B.comeDate,B.leaveDate,B.createDate,B.state,null,null,null FROM XL_ATTENDANCE B where B.state=1 and B.attendanceDate>=? and B.objId=? and B.attendanceType=1) C order by C.startDate desc desc limit ?,?";
