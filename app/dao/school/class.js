...
Class.listTeacherByClassId = function(classId, callback){
    mysqlUtil.query("select A.classId,A.isMaster,A.jobType,B.nickName,B.userName,B.custName,B.userId,concat('yunuser_',B.userId) as yunAccout,B.userUrl from XL_CLASS_TEACHER_REL A,XL_USER B where A.tUserId=B.userId and A.state=1 and A.classId=?", [classId], callback);
}
...
Class.findPrincipalByClassId = function(classId, callback){
    mysqlUtil.queryOne("select -1 as classId,-1 as isMaster,'校长' as jobType,C.nickName,C.userName,C.custName,C.userId,concat('yunuser_',C.userId) as yunAccout,C.userUrl from XL_SCHOOL A, XL_CLASS B, XL_USER C where A.schoolId=B.schoolId and A.sUserId=C.userId and B.classId=?", [classId], callback);
}
...
Class.findPrincipalBySchoolId = function(classId, callback){
    var sql = "select C.userId,C.userName,C.custName,C.nickName,A.schoolId,A.schoolName,concat('yunuser_',C.userId) as yunAccout, from XL_SCHOOL A, XL_CLASS B, XL_USER C where A.schoolId=B.schoolId and A.sUserId=C.userId and B.classId=?";
...
Class.listParentsByClassId = function(classId, callback){
    var sql = "select A.studentId,A.studentName,C.userId,C.userName,C.nickName,C.custName,concat('yunuser_',C.userId,'_',A.studentId) as yunAccout,,C.userUrl from XL_STUDENT A,XL_USER_STUDENT_REL B,XL_USER C where A.studentId=B.studentId and B.userId=C.userId and A.state=1 and B.state=1 and classId=? order by B.studentId";
